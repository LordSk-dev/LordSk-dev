...
Env vars:
  ...
  GH_LOGIN      user to summarise (default: LordSk-dev)
...
def sample_data():
    ...
    for w in range(52):
        w_days = []
        for d in range(7):
            dt = today - timedelta(days=(51 - w) * 7 + (6 - d))
            count = ((w * 3 + d * 7) % 11) if ((w + d) % 3 != 0) else 0
            d_obj = {"contributionCount": count, "date": dt.isoformat(), "weekday": d}
            w_days.append(d_obj)
            days_list.append(d_obj)
        weeks.append({"contributionDays": w_days})

    cal = {"totalContributions": 842, "weeks": weeks}
    user = {
        "contributionsCollection": {"contributionCalendar": cal},
        "repositories": {
            "nodes": [
                {"languages": {"edges": [{"size": 142000, "node": {"name": "TypeScript"}}, {"size": 89000, "node": {"name": "JavaScript"}}, {"size": 65000, "node": {"name": "Python"}}]}},
                {"languages": {"edges": [{"size": 110000, "node": {"name": "Rust"}}, {"size": 45000, "node": {"name": "Go"}}]}},
                {"languages": {"edges": [{"size": 95000, "node": {"name": "TypeScript"}}, {"size": 40000, "node": {"name": "C++"}}]}},
            ]
        }
    }
...
def draw_streak(s):
    ...
    for k, lab in (("current", "current streak"), ("longest", "longest streak")):
        ...
        span = (f"{pretty(r['start'])} &#8211; {pretty(r['end'])}"
                if r["length"] and r.get("start") and r.get("end") else "&#8212;")
...
def main():
    ...
    login = os.environ.get("GH_LOGIN", "LordSk-dev")
    out_dir = os.environ.get("OUT_DIR", ".")

    if token:
        print(f"Fetching live GraphQL data for {login}...")
        s = summarise(fetch(login, token))
    else:
        print(f"No GITHUB_TOKEN set. Generating initial stats for {login}...")
        s = sample_data()
